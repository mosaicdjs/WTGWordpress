...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-refixes="fn">
  ...
  <xsl:param name="id"></xsl:param>
       	<xsl:template match="//City">
			<h1><xsl:value-of select="@title"/> Nachtleben</h1>
			...
			<h2>Live-Musik in <xsl:value-of select="@title"/></h2>
			<xsl:apply-templates select="Content/Nightlife/CityNightlifeItem" mode="nightlifeItem">
				<xsl:with-param name="type">Live Music</xsl:with-param>
			</xsl:apply-templates>
			
			<h2>Klassische Musik in <xsl:value-of select="@title"/></h2>
			<xsl:apply-templates select="Content/Nightlife/CityNightlifeItem" mode="nightlifeItem">
				<xsl:with-param name="type">Classical Music</xsl:with-param>
			</xsl:apply-templates>
			
			<h2>Tanzen in <xsl:value-of select="@title"/></h2>
			<xsl:apply-templates select="Content/Nightlife/CityNightlifeItem" mode="nightlifeItem">
				<xsl:with-param name="type">Dance</xsl:with-param>
			</xsl:apply-templates>
			
			<h2>Theater in <xsl:value-of select="@title"/></h2>
			<xsl:apply-templates select="Content/Nightlife/CityNightlifeItem" mode="nightlifeItem">
				<xsl:with-param name="type">Theatre</xsl:with-param>
			</xsl:apply-templates>
			
			<h2>Musik und Tanz in <xsl:value-of select="@title"/></h2>
			<xsl:apply-templates select="Content/Nightlife/CityNightlifeItem" mode="nightlifeItem">
				<xsl:with-param name="type">Music und Dance</xsl:with-param>
			</xsl:apply-templates>
			
			<h2>Kultur in <xsl:value-of select="@title"/></h2>
			<xsl:apply-templates select="Content/Nightlife/CityNightlifeItem" mode="nightlifeItem">
