  ...
  <xsl:param name="id"></xsl:param>
       	<xsl:template match="//Region">
			...
			<div itemprop="text">
				...
				<xsl:if test="Content/PassportVisa/IdentityCards/text() != ''">
					<h3>Identity Cards</h3>
					<xsl:value-of select="Content/PassportVisa/IdentityCards/text()" disable-output-escaping="yes"/>
				</xsl:if>
				...
				<xsl:if test="Content/PassportVisa/PassportVisaNote/text() != ''">
					<h3>Passport Visa Note</h3>
					<xsl:value-of select="Content/PassportVisa/PassportVisaNote/text()" disable-output-escaping="yes"/>
				</xsl:if>
				...
				<xsl:if test="Content/PassportVisa/SchengenVisas/text() != ''">
					<h3>Schengen Visas</h3>
					<xsl:value-of select="Content/PassportVisa/SchengenVisas/text()" disable-output-escaping="yes"/>
				</xsl:if>
				...
				<xsl:if test="Content/PassportVisa/SufficientFunds/text() != ''">
					<h3>Sufficient Funds</h3>
					<xsl:value-of select="Content/PassportVisa/SufficientFunds/text()" disable-output-escaping="yes"/>
				</xsl:if>
				
				
				<xsl:if test="Content/PassportVisa/CompulsoryRegistration/text() != ''">
					<h3>Compulsary Registration</h3>
					<xsl:value-of select="Content/PassportVisa/CompulsoryRegistration/text()" disable-output-escaping="yes"/>
				</xsl:if>
				
				
				<xsl:if test="Content/PassportVisa/EntryDocuments/text() != ''">
					<h3>Entry Documents</h3>
					<xsl:value-of select="Content/PassportVisa/EntryDocuments/text()" disable-output-escaping="yes"/>
				</xsl:if>
				...
				<xsl:if test="Content/PassportVisa/ExitPermission/text() != ''">
					<h3>Exit Permission</h3>
					<xsl:value-of select="Content/PassportVisa/ExitPermission/text()" disable-output-escaping="yes"/>
				</xsl:if>
				...
				<b><i>This page reflects our understanding of current rules for the most common types of travel to the said country. However, please note that each authority sets and forces entry rules, hence we strongly recommend that you verify critical information with the relevant embassy before travel.</i></b>
				<h2>Embassies and tourist offices</h2>
				<xsl:for-each select="Content/ContactAddresses/Contacts/Contact">
					<h3><xsl:value-of select="@title"/></h3>
					<xsl:value-of select="./Intro" disable-output-escaping="yes"/><br/>
