...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-refixes="fn">
  ...
  <xsl:param name="id"></xsl:param>
       	<xsl:template match="//City">
			<h1>Sehenswürdigkeiten in <xsl:value-of select="@title"/></h1>
			
			<h2>Touristeninformationsbüro</h2>
			...
			<xsl:param name="importance"></xsl:param>
				...
				<xsl:if test="./Importance/text() = $importance">
					<div>
						...
						<b>Eintrittsgebühren: </b><xsl:value-of select="./AdmissionFees/text()" disable-output-escaping="yes"/>
						<b>Behindertengerechter Zugang: </b>
						<xsl:choose>
							<xsl:when test="./DisabledAccess/text() = '1'">Yah</xsl:when>
							<xsl:otherwise>Nein</xsl:otherwise>
						</xsl:choose>
						<br/>
						<b>UNESCO: </b>
						<xsl:choose>
							<xsl:when test="./Unesco/text() = '1'">Yah</xsl:when>
							<xsl:otherwise>Nein</xsl:otherwise>
						</xsl:choose>
						<br/>
					</div>
