<?php 
/*
* Template Name: Listing Management Form
*/
get_header('listing');
global $wpdb;
	...
	<div class="container">
		<div class="row box_item">';
			if (!is_user_logged_in())
			{
                ...
                echo '<div class="col-sm-6">'; $results = wp_login_form($args); echo '</div>';
                $payment = $_GET['payment'];
                echo $payment;
				if ($payment == 'paid88') {echo '<div class="col-sm-6"><p>'.get_field('post-payment_instructions').'</p>'.do_shortcode ( '[contact-form-7 id="16541" title="Listing Registration"]').'</div>'; }
				if ($payment == 'cancel44') { echo 'Payment process cancelled';}
				if ($payment == '')
				{
                    echo '<div class="col-sm-6">
                    
                    <p>'.get_field('pre-payment_instructions').'</p> 

					<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top"><input name="cmd" type="hidden" value="_s-xclick" />
					<input name="hosted_button_id" type="hidden" value="VVSWV8N6455NG" />
					<table>
                    <tbody>
					<tr>
					<td><input name="on0" type="hidden" value="Listing Types" />Select Listing Type</td>
                    </tr>
					<tr>
					<td><select name="os0" style="margin-top:10px; margin-bottom:10px">
					<option value="Standard">Standard : £0.01 GBP - monthly</option>
					<option value="Enhanced">Enhanced : £0.01 GBP - monthly</option>
					</select></td>
					</tr>
					</tbody>
					</table>
...
/*				echo '<div class="col-sm-6">'.do_shortcode ( '[contact-form-7 id="16541" title="Listing Registration"]').'</div>'; */
			}
			else	
			{
