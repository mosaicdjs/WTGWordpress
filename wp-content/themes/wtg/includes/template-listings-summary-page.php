...
<script>
    ...
    { 
        ...
        $infoWinContent = '<h3>'.get_the_title($postID).'</h3>';
        echo "var label = '';";
...
echo '<h2><i class="icon icon_1"></i>'.$pageType.' Directory</h2>'; 
    ...
    if ($listingStandard->have_posts())
    {
        ...
        <div class="row">
            <div class="col-sm-12">
                <h2>More '.$pageType.'s in '.get_the_title($postIDLocation).' to consider</h2>
                <div class="row" style="background-color:#e6e3ed; padding-top:25px; padding-bottom:25px; margin-bottom:25px">
                    <div class="col-sm-12"><strong>Filter By Price:</strong> 
                        <a href="'.get_the_permalink($currentPageID).'?location='.$postIDLocation.'&type='.$pageType.'&price=Cheap">Cheap</a>
                        <a href="'.get_the_permalink($currentPageID).'?location='.$postIDLocation.'&type='.$pageType.'&price=Moderate">Moderate</a>
                        <a href="'.get_the_permalink($currentPageID).'?location='.$postIDLocation.'&type='.$pageType.'&price=Luxury">Luxury</a>
                    </div>
                </div>
            </div>
        </div>';
        while ($listingStandard->have_posts())
        {
            ...
            <div class="row box_item" style="margin-bottom:25px; padding-top:10px; border:1px solid #1e73be;">
                <div class="col-sm-4"><img src="'.get_field('listing_main_image', $post->ID).'" alt="" class="img-responsive"></div> 
                ...
                <div class="col-sm-8">
                    ...
                    <p>'.get_field('listing_introduction', $post->ID).'</p>
                     <div class="row">
                        ...
                        <div class="col-sm-5">
                         <p style="font-size:12px;">Tel:   '.get_field('listing_telephone', $post->ID).'<br />
                            Email: '.get_field('listing_email', $post->ID).'<br />
